...
export const Component = function DiaryRecord(): JSX.Element {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3000/thoughtmodificationrecord", {
        params: {
    ...
    <MaterialCssVarsProvider theme={{ [THEME_ID]: materialTheme }}>
      <LocalizationProvider dateAdapter={AdapterDayjs}>
        <Container sx={{ py: 2 }}>
          ...
          >
            User Thought Modification Record Log
          </Typography>
          ...
          {userHistory.map((item, index) => (
            <Card key={index} sx={{ marginBottom: 2 }}>
              <CardContent>
                ...
                <Typography style={fontStyle}>
                  Thought: {item.content.thought}
                </Typography>
                <Typography style={fontStyle}>
                  Emotion: {item.content.emotion}
                </Typography>
                <Typography style={fontStyle}>
                  Intensity: {item.content.intensity}
                </Typography>
                <Typography style={fontStyle}>
                  Positive thought: {item.content.positiveThought}
                </Typography>
                <Typography style={fontStyle}>
                  Rewrite Event: {item.content.rewriteEvent}
                </Typography>
                <Typography
                  style={{
                    fontFamily: "Didact Gothic",
